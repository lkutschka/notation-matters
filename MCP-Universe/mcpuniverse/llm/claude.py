...
class ClaudeModel(BaseLLM):
    ...
    def _generate(
        ...
        if response_format is None:
            ...
            self._last_usage = {
                "prompt_tokens": getattr(chat.usage, 'input_tokens', 0) or 0,
                "completion_tokens": getattr(chat.usage, 'output_tokens', 0) or 0,
                "total_tokens": (getattr(chat.usage, 'input_tokens', 0) or 0) +
                                (getattr(chat.usage, 'output_tokens', 0) or 0),
                "cost_usd": 0.0,
            }
        ...
        # Capture API usage for structured output calls too
        self._last_usage = {
            "prompt_tokens": getattr(chat.usage, 'input_tokens', 0) or 0,
            "completion_tokens": getattr(chat.usage, 'output_tokens', 0) or 0,
            "total_tokens": (getattr(chat.usage, 'input_tokens', 0) or 0) +
