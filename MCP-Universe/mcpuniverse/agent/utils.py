...
def get_tools_description(tools: Dict[str, List[Tool]]) -> str:
    ...
    for server_name, tool_list in tools.items():
        for tool in tool_list:
            ...
            print(f"Generated description for tool '{tool.name}' from server '{server_name}':\n{description}\n")
    return "\n\n".join(descriptions).strip()


def get_tools_description_formatted(
    tools: Dict[str, List[Tool]],
    format_converter: Optional[Any] = None,
) -> str:
    """
    Generate a formatted description of tools using the specified format converter.

    When format is JSON (or no converter), delegates to get_tools_description().
    When format is TOON/TRON, uses the converter's serialize_tools().

    Args:
        tools: A dictionary of tools, where keys are server names and values are lists of Tool objects.
        format_converter: A ToolFormatConverter instance, or None for default YAML behavior.
    ...
    from mcpuniverse.agent.format_converter import ToolFormat
    if format_converter.format == ToolFormat.JSON:
        return get_tools_description(tools)
    return format_converter.serialize_tools(tools)
...
def build_system_prompt(
    ...
    **kwargs,
    ...
    Args:
        ...
        format_converter: Optional ToolFormatConverter for non-JSON formats.
    ...
    tools_prompt = ""
    tools_description = get_tools_description_formatted(tools, format_converter) if tools else ""
